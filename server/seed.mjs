    ...
    const appointment = repoFor('Appointment').create(
      {
        ...
        notes: 'Initial exercise physiology consultation (synthetic seed data)',
        status: 'completed',
      },
      clinician.email,
    );
      ...
      repoFor('ClientAssessment').create(
        {
          ...
          notes: 'Recorded during initial consultation (synthetic seed data)',
      ...
      repoFor('SOAPNote').create(
        {
          ...
          subjective: 'Client reports steady progress with home exercise programme (synthetic seed data).',
    ...
    repoFor('ClientReport').create(
      {
        ...
        report_data: {
          summary: 'Synthetic seed GP summary report.',
          assessments_included: clientAssessments.map((ca) => ca.id),
        },
        html_content: `<p>Synthetic seed GP summary report for ${fullName}.</p>`,
        notes: 'Generated by server/seed.mjs',
      },
      clinician.email,
    );
    ...
    repoFor('SavedReport').create(
      {
        ...
        client_id: client.id,
        report_type: 'gp_summary',
        report_name: `Saved GP Summary - ${fullName}`,
        report_date: isoDate(seedYear, seedMonth, seedDay),
        ...
        section_content: { summary: 'Synthetic seed saved report content.' },
        active_sections: ['summary'],
        report_html: `<p>Synthetic seed saved report for ${fullName}.</p>`,
        status: 'draft',
      },
      clinician.email,
    );
