...
const ADMIN_NOTIFY_TO = () => process.env.EMAIL_ADMIN_NOTIFY || 'admin@assesssuite.com';
...
export async function sendEmail({ to, subject, text, html }) {
  try {
    if (outbox) outbox.record({ to, subject, body: text });
  } catch (err) {
    console.log('[email] outbox record failed:', err.message);
  }
  if (!emailEnabled()) return { recorded: true, sent: false };
  try {
...
${bodyHtml}
<p style="color:#94a3b8;font-size:12px;margin-top:28px">AssessSuite Clinical — a product of Assess Suite Pty Ltd (ABN 53 694 044 481). This is an automated message; replies reach ${EMAIL_REPLY_TO()}.</p>
</body></html>`;
}
...
export function otpEmail(code) {
  return {
    ...
    html: htmlWrap(
      'Verify your email',
      `<p>Your AssessSuite verification code is:</p><p style="font-size:28px;font-weight:bold;letter-spacing:6px;color:#0f172a">${code}</p><p>The code expires in 10 minutes. If you did not request this, you can ignore this email.</p>`,
    ),
  };
}
...
export function welcomeEmail(name) {
  ...
  return {
    ...
    html: htmlWrap(
      ...
      `<p>Welcome${who},</p><p>Your AssessSuite account is now active. You can sign in and begin using the platform.</p><p>Support: ${EMAIL_REPLY_TO()} | 1800 317 553 (Mon–Thu, 10 am–2 pm AEST)</p>`,
    ),
  };
}
...
export function adminNotifyEmail(newUserEmail) {
  return {
    ...
    html: htmlWrap(
      'New registration',
      `<p>A new account has registered on AssessSuite: <strong>${newUserEmail}</strong></p><p>The account is pending; it activates automatically on successful subscription payment.</p>`,
    ),
  };
}
...
export function inviteEmail(role) {
  return {
    ...
    html: htmlWrap(
      ...
      `<p>You have been invited to AssessSuite (role: <strong>${role}</strong>).</p><p>Create your login at the AssessSuite site using this email address.</p>`,
    ),
  };
}
