// Ported from base44/functions/fixHasTestRunnerFlags/entry.ts.
// Admin-only: reconciles Assessment.has_test_runner against the same
// name-pattern detection logic used client-side by TestRunner.js /
// TestRunnerExtras.js, batching writes 20 at a time exactly as entry.ts does.
...
export default async function fixHasTestRunnerFlags(ctx) {
  ...
  for (const assessment of allAssessments) {
    ...
    if (should && !assessment.has_test_runner) {
      updates.push(entities.Assessment.update(assessment.id, { has_test_runner: true }));
      updated++;
