...
// ---------------------------------------------------------------------------
// Boot H — the SELFTEST loopback carve-out (server/llm.mjs resolveChatTest
// BaseUrl) must require an obviously synthetic OPENAI_API_KEY, not merely a
// validated loopback URL: a shell-inherited real key must never be honoured
// under SELFTEST=1, even with OPENAI_CHAT_TEST_BASE_URL explicitly wired.
// ---------------------------------------------------------------------------

test('Boot H: a non-synthetic OPENAI_API_KEY is never honoured by the SELFTEST loopback carve-out', async () => {
  const fakeChat3 = await startFakeOpenAIChat();
  const server = await startTestServer({
    GENERAL_CLINICAL_LLM_ENABLED: '1',
    LLM_REQUIRED: '0',
  ...
  try {
    ...
    const result = await invokeLlm(server, admin, PLAIN_PROMPT_BODY);
    assert.equal(result.status, 200, result.text);
    assert.match(result.body, /placeholder narrative content generated by the local InvokeLLM mock/i, result.text);
    assert.equal(fakeChat3.calls.length, 0, 'a non-synthetic key must never reach the loopback test provider');
  } finally {
    await server.stop();
    await fakeChat3.stop();
  }
});
