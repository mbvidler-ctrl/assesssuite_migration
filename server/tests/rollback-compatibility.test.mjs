...
test('R00 rollback differs only by extraction and legal compatibility while generic clinical AI stays disabled', () => {
  ...
  assert.equal(candidate.get('env.GENERAL_CLINICAL_LLM_ENABLED'), '"0"');
  assert.equal(rollback.get('env.GENERAL_CLINICAL_LLM_ENABLED'), '"0"');
  assert.equal(candidate.get('env.CORE_V1_SANDBOX_ENABLED'), '"0"');
  assert.equal(rollback.get('env.CORE_V1_SANDBOX_ENABLED'), '"0"');
  assert.equal(
  ...
  assert.deepEqual(
    ...
    'rollback may differ only by the reviewed extraction switch plus legal compatibility allowlist',
  );
});
