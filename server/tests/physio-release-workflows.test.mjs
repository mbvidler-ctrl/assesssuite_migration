...
test('R1 comparison deployment is immutable, snapshot-isolated, fully tested and non-billing', () => {
  ...
  assert.match(source, /R1_STAGE2_TEST_SHA: 27cec6656472788f491cdb447c996bcf4a0b4bb1/);
  assert.match(source, /R1_PRE_SHARED_SHA: e3957f34f0585209393237dca26c16755d4501f8/);
  ...
  assert.match(source, /git rev-list --first-parent --reverse "\$R1_BASELINE_SHA\.\.\$R1_COMPARISON_SHA"/);
  assert.match(source, /diff -u "\$expected_chain" "\$actual_chain"/);
  ...
  assert.match(source, /flyctl volumes snapshots list "\$R1_VOLUME_ID"/);
  ...
  assert.match(source, /\.capabilities\.general_clinical_llm/);
  assert.match(source, /\.capabilities\.transcription/);
  assert.match(source, /\.capabilities\.document_extraction/);
  ...
  assert.match(source, /--arg upgrade_sha "\$R1_PRE_SHARED_SHA"/);
  assert.match(source, /--arg live_pre_ai_sha "\$R1_PRE_AI_ENABLE_SHA"/);
  assert.match(source, /image_ref\.labels\.ASSESSSUITE_SOURCE_SHA == \$upgrade_sha or/);
  assert.match(source, /image_ref\.labels\.ASSESSSUITE_SOURCE_SHA == \$live_pre_ai_sha or/);
