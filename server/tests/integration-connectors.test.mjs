...
test('integration credential vault encrypts secrets and binds them to organisation and provider', () => {
  ...
  const credentials = { client_id: 'client-1234', client_secret: 'test-secret-do-not-leak' };
  ...
  assert.doesNotMatch(envelope, /client-1234|test-secret-do-not-leak/);
...
test('Halaxy client proves OAuth and Patient permission without returning patient records', async () => {
  ...
  const fetchImpl = async (url, options) => {
    ...
    if (String(url).endsWith('/oauth/token')) {
      return jsonResponse({ token_type: 'fixture-bearer', expires_in: 900, access_token: 'test-halaxy-token' });
    }
    return jsonResponse(
  ...
  assert.equal(calls[1].options.headers.Authorization, 'Bearer test-halaxy-token');
