...
const FLAGS = [
  {
    ...
    serverGates: [
      ...
      },
      {
        ...
        symbol: 'openRegistrationAvailable / publicRegistrationPosture',
        route: 'GET /api/apps/public/prod/public-settings/by-id/:appId (publication of the enforced posture)',
        effectWhenOff: 'public_settings.registration is published as { mode: "invitation_only", open: false }.',
      },
    ],
    reportedVia: 'GET /api/apps/public/prod/public-settings/by-id/:appId → public_settings.registration (mirrors the same profession-specific server gate)',
    clientDetector: null,
    detectorNote:
      'Registration is a full-page auth flow rather than an InvokeLLM/ExtractDataFromUploadedFile call site; ' +
      'src/lib/registrationAccess.js is the one shared client decision helper, so a marker-based detector would ' +
      'find nothing meaningful to count. The server-gate table above is the complete blast-radius record for this flag.',
    clientSurfaces: [],
