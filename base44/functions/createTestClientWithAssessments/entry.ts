...
function generateAssessmentData(assessment) {
  ...
  } else if (name.includes('four stage balance') || name.includes('4-stage balance') || name.includes('4 stage balance')) {
    resultValue = 3;
    additionalData = {
  ...
  // â”€â”€ STRENGTH â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    additionalData = {
      ...
      interpretation_summary: `Client achieved ${resultValue}kg on the leg press, representing ${Math.round(resultValue / 80 * 100) / 100}x body mass. Classified as ${resultValue >= 100 ? 'Intermediate' : 'Novice'} for age-matched peers.`
    };
  ...
  // â”€â”€ PAIN / FUNCTION / QUESTIONNAIRES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    additionalData = {
      ...
      responses: Array(9).fill(0).map(() => rndInt(0, 2))
    };
  } else if (name.includes('gad-7') || name.includes('gad7')) {
    resultValue = rndInt(2, 9);
    additionalData = {
      measurement_type: 'questionnaire',
      responses: Array(7).fill(0).map(() => rndInt(0, 2))
    };
    ...
    additionalData = {
      measurement_type: 'questionnaire',
      responses: Array(10).fill(0).map(() => rndInt(1, 4))
    };
...
Deno.serve(async (req) => {
  try {
    ...
    const fakeClient = await base44.entities.Client.create({
      org_id: orgId,
    ...
    console.log('Created fake client:', fakeClient.id);
    ...
    const soapNote = await base44.entities.SOAPNote.create({
      org_id: orgId,
      client_id: fakeClient.id,
    ...
    for (const assessment of allAssessments) {
      ...
      try {
        ...
        const clientAssessment = await base44.entities.ClientAssessment.create({
          org_id: orgId,
          client_id: fakeClient.id,
        ...
        console.log(`Processed: ${assessment.name}`);
      } catch (error) {
