...
const validatorSha256 = crypto.createHash('sha256')
  .update(fs.readFileSync(scriptPath, 'utf8').replaceAll('\r\n', '\n'))
  .digest('hex');
...
function validateWorkflow(rawSource) {
  ...
  for (const marker of [
    ...
    "read_public_version 'https://app.assesssuite.com'",
    ...
    'if (apexSha !== flySha)',
    "schema_version: 'assesssuite-production-state-snapshot-v2'",
    "result: 'PASS'",
    ...
    'machine_count: 1',
    'volume_count: 2',
    "active_volume_name: 'assesssuite_data_r12'",
    'active_volume_id: volumeId',
    'active_volume_size_gb: 3',
    'active_volume_encrypted: true',
    'active_volume_scheduled_snapshots: true',
    'active_volume_snapshot_retention_days: 5',
    "legacy_volume_name: 'assesssuite_data'",
    'legacy_volume_id: legacyVolumeId',
    'legacy_volume_size_gb: 3',
    'legacy_volume_encrypted: true',
    'legacy_volume_attached_machine_id: null',
    'legacy_volume_scheduled_snapshots: true',
    'legacy_volume_snapshot_retention_days: 5',
...
if (selftest) {
  const mutations = [
    ...
    ['topology-count-bypass', 'if (machines.length !== 1 || volumes.length !== 2)', 'if (false)'],
    ['volume-name-uniqueness-bypass', 'new Set(volumes.map((row) => row?.name)).size !== 2', 'false'],
    ['active-volume-selector-changed', "row?.name === 'assesssuite_data_r12'", "row?.name === 'assesssuite_data'"],
    ['legacy-detachment-bypass', 'legacyVolume.attached_machine_id !== null', 'false'],
    ['legacy-allocation-detachment-bypass', 'legacyVolume.attached_alloc_id !== null', 'false'],
    ['legacy-encryption-bypass', 'legacyVolume.encrypted !== true', 'false'],
    ['legacy-snapshot-retention-bypass', 'legacyVolume.snapshot_retention !== 5', 'false'],
    ['legacy-scheduled-snapshot-bypass', 'legacyVolume.auto_backup_enabled !== true', 'false'],
    ['active-mount-volume-bypass', 'mounts[0]?.volume !== volumeId', 'false'],
    ['active-mount-name-bypass', 'mounts[0]?.name !== volume.name', 'false'],
    ['active-mount-encryption-bypass', 'mounts[0]?.encrypted !== true', 'false'],
