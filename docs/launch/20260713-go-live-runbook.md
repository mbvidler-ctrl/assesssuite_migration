...
**Deployment corridor (always):** use two reviewed dispatches from the same immutable default-branch revision. First, `production-prepare-release.yml` gates the exact SHA, publishes the candidate image without changing the live release, Machine or volume, proves exact-image compatibility and seals the candidate digest plus bounded deploy-bundle evidence. Only after those immutable outputs are independently recorded in the production intent may `production-deploy.yml` consume the exact preparation-run, artifact and manifest identities together with the refreshed current-state, topology, snapshot and rollback inputs. The enabled `fly.production.toml` and extraction-disabled `fly.rollback.production.toml` are reviewed in that same revision; no mutable rollback branch is permitted. The sealed workflow is the only place permitted to invoke `fly deploy --strategy immediate --ha=false --update-only`; specifically, that is the reviewed `production-deploy workflow`; never run that command locally. The single-volume topology must never use blue-green deployment or scale beyond one machine (a second machine gets an empty volume and splits the data).
...
- Deploys: `production-prepare-release.yml` publishes and seals an exact candidate without changing the live release, Machine or volume; only the subsequent trusted `production-deploy workflow` uses `--strategy immediate` after validating that immutable handoff. Expect a brief (~10-20 s) outage while the existing volume persists. The sealed deploy job performs the names-only required-secret preflight and aborts before mutation if the exact production secret-name set is incomplete. Do not run `npm run deploy:prod`; it is intentionally fail-closed. This gate exists because Fly secrets are app-specific and a credential present on another app proves nothing about `assesssuite-production`.
- `unimatter-demo` (the reseeding demo) is unchanged and remains the safe demonstration surface.
- The v32 production candidate enables capped transcription through reviewed configuration and dedicated assurance. Compatibility rollback keeps it disabled because the retained image predates the durable per-user usage ledger; it is never an opaque-secret toggle. General clinical AI remains enabled in both postures.
