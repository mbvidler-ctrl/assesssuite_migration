...
- the dispatch-frozen previous image is reverified against `R`, then deployed only with the reviewed `fly.rollback.production.toml` from `C`, which disables adult and under-age document extraction and keeps generic clinical AI drafting (`GENERAL_CLINICAL_LLM_ENABLED`) at its reviewed disabled default.
...
The reviewed rollback configuration has one deliberate capability difference from the candidate configuration: adult document extraction is disabled (under-age extraction and generic clinical AI drafting are disabled in both configurations). The rollback legal-compatibility allowlist is the other reviewed configuration difference. The exact previous image must pass the current forward/rollback compatibility proof before its digest can enter the deploy bundle.
...
Require exact current-state and two-volume topology guards, current-main rollback-config provenance, extraction disabled for adult and under-age data and generic clinical AI drafting kept disabled, immutable digest deployment, unchanged volumes and `R` from both public version endpoints. Do not use `fly releases rollback`: only the exact dispatch-frozen image that passed the preparation receipt chain is authorised.
