...
| CL-CAT-WRITEBACK | 1/3 | catalogue_contamination | `AssessmentAudit.jsx:847`; `AdminApprovals.jsx:244/282/419/1080` | LLM/admin content (references, contraindications, scoring) persisted into the Assessment catalogue with no DOI verification | LLM-persisted-to-catalogue | defect | Gate every `Assessment.update` write-back behind the citation-verification service before persistence; log provenance. | 3 | open |
| CL-CLICKABLE-REFS | 3 | reference_DOI | `ClickableReferences.jsx` | Any citation-shaped string rendered as an authoritative clickable DOI/PMID link with no existence or support check | presentation layer | defect | Render a "verified" badge only when the verification service confirms; otherwise mark unverified. | 3 | open |
...
| CL-MEDALERTS | 1 | contraindication | `client/MedicationAlerts.jsx:68` | Per-medication exercise-safety alerts from an "expert pharmacologist" prompt, parametric memory, no citation | LLM parametric | signoff | Ground against RxNorm/openFDA + clinician sign-off; never present as verified. | 1 | open |
| CL-TX-PROTOCOL-REFS | 1 | reference_DOI | `TreatmentProtocols.jsx:252` | LLM protocol requests DOIs; `validateReferences` checks only that the DOI RESOLVES, not that it supports the claim | LLM+web | flag | Replace existence-only check with title/author/year cross-match via the verification service; visible drop count. | 3 | open |
