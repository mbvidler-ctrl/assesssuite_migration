...
**Live-platform closure (2026-07-03).** The equivalent fix was applied directly to Brenton's live app (reference `ASX-SEC-20260703-01`), independently verified byte-identical via the connector, and — since a live test credential could not be generated by the agent and none was supplied — the authenticated-non-admin and page-level checks were instead run as real users (real login, real session, real browser) against a disposable local clone of the same, confirmed-identical code. All four functions returned 403 for a real non-admin session and passed cleanly for admin; both pages showed "Admin Access Required" with zero data-call leakage for non-admin and loaded fully for admin. See `20260703-clone-based-real-user-test-results.md` in the AssessSuite service-delivery folder.
