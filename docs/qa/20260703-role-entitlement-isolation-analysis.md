...
A follow-up review, prompted by preparing a client-facing patch prompt, checked the remaining ten backend functions individually rather than by name-pattern inference and found a fourth unguarded function that item 2 above missed: **`getMissingTestRunners`** has no caller-identity or role check of any kind — it does not even call `auth.me()` — identical in kind to `auditAssessmentIssues`, though its data (the global Assessment catalogue) is less sensitive than client-specific records. The remediation queue item 2 above should read four functions, not three: `auditAssessmentIssues`, `createTestClientWithAssessments`, `verifyTestAssessmentData`, `getMissingTestRunners`. Guard applied in the shim (commit `66b7fa5`); selftest 66/66; gate-tests 13/13. The other seven maintenance functions (`assignOrganizations`, `fixUserOrganizations`, `fixMissingOrgIds`, `createMissingAssessments`, `enableMissingTestRunners`, `fixHasTestRunnerFlags`) were individually verified to already carry the correct admin-only guard in the captured source (one, `fixHasTestRunnerFlags`, uses a slightly different error message — "Admin only" instead of "Forbidden: Admin access required" — a cosmetic inconsistency, not a vulnerability, not remediated). This correction is also reflected in the Launch Gate Matrix, the handover note, the engineering notes, and the run-state file.
