...
- `public_settings.capabilities` (added, see server/capabilities.mjs — the single source of truth for both this publication and the enforcing endpoints):
  ...
  - `transcription_enabled` is retained permanently as the legacy raw-switch alias and keeps its exact configured-value expression. `capabilities.transcription` is provider-aware: it can publish `unconfigured` when the switch is on but production requires a missing provider. The values also diverge under the deliberate `SELFTEST=1` offline mock posture. Current UI surfaces use the capability block; older bundles retain the raw alias for compatibility.
