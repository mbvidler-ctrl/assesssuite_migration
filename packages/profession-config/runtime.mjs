...
const definitions = [physiotherapyDefinition];
// The sealed Physio production tree deliberately omits the EP definition.
// Existing EP and development/test composition load it through this bounded
// target branch; a Physio image cannot silently fall back because its explicit
// PROFESSION is resolved before DEFAULT_PROFESSION_ID is considered.
if (process.env.NODE_ENV !== 'production' || process.env.PROFESSION !== 'physio') {
  const epUrl = new URL('./professions/exercise-physiology.mjs', import.meta.url);
  definitions.unshift((await import(epUrl.href)).default);
}
...
for (const field of ['shellId', 'appId', 'localAppPort', 'localServerPort', 'dataFile', 'intendedAppHost']) {
...
export function getProfession(id) {
  ...
  if (!definition) {
    throw new TypeError(`unknown profession "${String(id)}"; expected one of ${PROFESSION_IDS.join(', ')}`);
  }
  return definition;
}

export function resolveProfession(environment = process.env) {
  const requested = typeof environment.PROFESSION === 'string' ? environment.PROFESSION.trim() : '';
  return requested ? getProfession(requested) : getProfession(DEFAULT_PROFESSION_ID);
}

export function resolveActiveProfessionContract(environment = process.env) {
  ...
  if (appId !== profession.deployment.appId) {
    throw new TypeError(
      `DEFAULT_APP_ID="${appId}" does not match PROFESSION="${profession.id}" (${profession.deployment.appId})`,
    );
  }
...
export function toPublicProfession(definitionOrId) {
  ...
  return {
    ...
    signup: { ...definition.signup, managementRoles: [...definition.signup.managementRoles] },
    navigation: {
