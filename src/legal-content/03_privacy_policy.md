...
**Controlled revision:** 4 August 2026, revision 2026-08-04.1, authorised by mission UM-AUTO-20260801-ASSESSSUITE-SPLIT-HOSTING-ANALYTICS
...
The first-party measurement does not create raw analytics-event rows. Each accepted occurrence increments only the relevant daily total in the AssessSuite Fly database. The aggregate measurement table and dashboard do not contain or derive IP address, user agent, referrer, acquisition source, URL, path, query string, exact event time, user, account, session or organisation identifier, clinical or payment information, or free text. A public-site page-load total counts page-load occurrences, not people or unique visitors. The public dashboard presents aggregate totals only. Vercel Web Analytics is a separate provider dataset with the narrower public-site scope disclosed above.
...
- measure the four disclosed first-party daily operational totals and present them through a public aggregate-only dashboard;
...
The four first-party totals are not used to identify or profile a person, attribute activity to a customer or practice, reconstruct a journey, measure an acquisition source or infer a health, clinical or payment activity. They do not estimate unique visitors. The client dashboard is public and aggregate-only. A separate owner dashboard remains access-controlled and may contain operational diagnostics and future-option analysis, but it must not expose Patient Data or person-level analytics.
...
The first-party aggregate measurement does not add an analytics provider. Its only measurement records are the four Australia/Brisbane daily totals held with the AssessSuite application database on Fly.io. Vercel Web Analytics is separately enabled for the public homepage and approved public legal pages and receives none of those first-party measurement events.
...
The first-party measurement retains only daily aggregate count records in the AssessSuite Fly database; it does not retain a raw measurement-event history from which a person, account, session, organisation, route, source or exact event time could be reconstructed.
