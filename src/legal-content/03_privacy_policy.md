...
**Controlled revision:** 2 August 2026, revision 2026-08-02.3, authorised by mission UM-AUTO-20260801-ASSESSSUITE-SPLIT-HOSTING-ANALYTICS
...
- after verified production activation, four first-party aggregate operational totals grouped only by Australia/Brisbane calendar day: public-site page loads, successful sign-ins, newly verified accounts and authenticated application opens;
- during a brief period on 2 August 2026, Vercel Web Analytics on the production public homepage and public legal pages may have processed the public page origin and path, event time, a request-derived daily visitor estimate, external referrer information and coarse country, browser, operating-system and device categories. The measured page URL query string and fragment were removed, but later verification showed that the provider could append the external referring URL after that page-URL filter;
- cookie and tracking information described in the Cookie, Analytics and Tracking Notice.

Vercel Web Analytics is not currently enabled. It was active from approximately 12:25 pm to 3:10 pm AEST on 2 August 2026 and was then disabled; the analytics component was removed from the deployed public website. Vercel reports that the project received data during that interval, but AssessSuite has not established that any actual Patient Data was included. The service was not authorised for Patient Data and was not loaded in the authenticated application or on account, clinical, patient, payment, upload, function or API routes. No custom events, UTM reporting, feature-flag analytics, session replay, advertising analytics, Analytics Plus or analytics drain were enabled.

The replacement measurement does not create raw analytics-event rows. Each accepted occurrence increments only the relevant daily total in the AssessSuite Fly database. The aggregate measurement table and dashboard do not contain or derive IP address, user agent, referrer, acquisition source, URL, path, query string, exact event time, user, account, session or organisation identifier, clinical or payment information, or free text. A public-site page-load total counts page-load occurrences, not people or unique visitors. The private dashboard presents aggregate totals only.
...
- after verified production activation, measure the four disclosed first-party daily operational totals and present them through a private aggregate-only dashboard;
...
The first-party aggregate measurement does not add an analytics provider. Its only measurement records are the four Australia/Brisbane daily totals held with the AssessSuite application database on Fly.io. Vercel Web Analytics remains disabled and receives none of those first-party measurement events.
