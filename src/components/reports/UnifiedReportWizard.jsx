...
import { LEGACY_REPORT_COMPATIBILITY_VERSION } from "@/lib/reports/core";

/**
 * The shared JavaScript UI wrappers pre-date repository-wide checkJs and are
 * inferred as ref-only components. These narrow aliases describe the runtime
 * props used here without weakening checking for the rest of this module.
 *
 * @typedef {{ className?: string, children?: React.ReactNode }} ClassedChildrenProps
 * @typedef {{ className?: string, value?: number }} ReportProgressProps
 * @typedef {{
 *   id?: string,
 *   assessment_id?: string,
 *   name?: string,
 *   unit_of_measure?: string,
 *   normative_direction?: string | null,
 *   status?: string,
 *   result_value?: unknown,
 *   additional_data?: { soap_text?: unknown } | null,
 *   assessment_date?: string,
 *   created_date?: string,
 * }} ReportAssessmentRecord
 */
const ReportDialogContent = /** @type {React.ComponentType<ClassedChildrenProps>} */ (
  /** @type {unknown} */ (DialogContent)
);
const ReportDialogTitle = /** @type {React.ComponentType<ClassedChildrenProps>} */ (
  /** @type {unknown} */ (DialogTitle)
);
const ReportProgress = /** @type {React.ComponentType<ReportProgressProps>} */ (
...
// Convert an ASCII / markdown pipe table in generated text into a real HTML
// table so columns align in printed output; leaves prose as paragraphs.
  ...
  while (i < lines.length) {
    if (lines[i].includes("|") && (lines[i + 1] || "").includes("|")) {
      ...
      if (rows.length) {
        ...
        out.push(
          `<table class="outcome"><thead><tr>${head.map((h) => `<th>${esc(h)}</th>`).join("")}</tr></thead>` +
          `<tbody>${bodyRows.map((r) => `<tr>${r.map((c) => `<td>${esc(c)}</td>`).join("")}</tr>`).join("")}</tbody></table>`
        );
      }
    } else {
      const para = [];
      while (i < lines.length && !(lines[i].includes("|") && (lines[i + 1] || "").includes("|"))) { para.push(lines[i]); i++; }
...
function buildGpLetterHtml(template, activeSections, content, client, clinician, outcomeAssessments) {
  ...
  const drName = gpName ? (/^dr\.?\s/i.test(gpName) ? gpName : `Dr ${gpName}`) : "";
  const salutation = drName ? `Dear ${drName},` : "Dear Doctor,";
  ...
  // The GP letter has no section headings, so the footer sentence carries the
  // whole disclosure. Both handleSave and handlePrint consume this output, so
  // the persisted report_html and the printed document disclose identically.
...
<style>
  body{font-family:Georgia,'Times New Roman',serif;font-size:13px;color:#1e293b;margin:0;padding:48px;line-height:1.6;}
  .lh{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #1a56db;padding-bottom:12px;margin-bottom:20px;}
  .lh h1{font-size:18px;color:#1a56db;margin:0;} .lh .cl{font-size:11px;color:#555;text-align:right;}
  .meta{font-size:12px;color:#334155;margin-bottom:16px;}
  .recip{margin-bottom:16px;font-size:12px;} .recip div{margin:1px 0;}
  .re{font-weight:bold;margin:16px 0;}
  .lp{margin:0 0 12px;} .lp .lead{font-weight:bold;color:#1a56db;} .lp p{margin:2px 0;}
  ...
  .signoff{margin-top:28px;} .signoff .sig-space{height:36px;}
  .footer{margin-top:36px;border-top:1px solid #e2e8f0;padding-top:8px;font-size:10px;color:#999;text-align:center;}
  @media print{body{padding:24px;}}
</style></head><body data-core-lifecycle="draft">
<div class="lh"><h1>${(clinician && clinician.clinic_name) || "AssessSuite"}</h1><div class="cl">${clinician ? (clinician.clinic_address || "") : ""}${clinician && clinician.clinic_phone ? `<br/>${clinician.clinic_phone}` : ""}</div></div>
<div class="meta">${today}</div>
<div class="recip">${drName ? `<div>${drName}</div>` : ""}${gpClinic ? `<div>${gpClinic}</div>` : ""}${gpAddress ? `<div>${gpAddress}</div>` : ""}</div>
<div>${salutation}</div>
<div class="re">RE: ${client.full_name} — DOB ${clientDOB}${age ? ` (${age} years)` : ""}${client.funding_source ? ` — ${client.funding_source.toUpperCase()}` : ""}</div>
<div data-assess-body>${body}</div>
<div class="signoff">Yours sincerely,<div class="sig-space"></div>${clinicianLine}</div>
<div class="footer">${anyAi ? `${AI_LETTER_DISCLOSURE_SENTENCE}<br/>` : ""}Generated by AssessSuite · Confidential Clinical Document</div>
</body></html>`;
}
...
<style>
  ...
  .ai-tag{font-size:11px;color:#92400e;background:#fef3c7;border:1px solid #fcd34d;border-radius:4px;padding:1px 5px;font-weight:normal;}
...
<div data-assess-body>${activeSections.filter(s => !s.toLowerCase().includes('signature') && !s.toLowerCase().includes('attachment')).map(section => {
  const text = content[section];
  const tableHtml = section === outcomeSection ? outcomeHtml : '';
  if (!text?.trim() && !tableHtml) return '';
...
<div class="footer">${anyAi ? `${AI_REPORT_DISCLOSURE_SENTENCE}<br/>` : ''}Generated by AssessSuite · Confidential Clinical Document</div>
</body></html>`;
}
...
export default function UnifiedReportWizard({ isOpen, onClose, client, clientData, clinician, existingReport, reportType: preselectedReportType }) {
  ...
  const STEPS = isEditing
    ? ["Edit Sections", "Draft Review"]
    : ["Report Type", "Date Range", "Select Assessments", "Sections", "Draft Review"];
  ...
  useEffect(() => {
    ...
    const fetchAssessments = async () => {
      ...
      try {
        ...
        /** @type {Record<string, ReportAssessmentRecord>} */
        const libraryMap = {};
        const libraryRows = /** @type {ReportAssessmentRecord[]} */ (assessmentLibrary || []);
        libraryRows.forEach((assessment) => {
          if (assessment.id) libraryMap[assessment.id] = assessment;
        });
        const resultRows = /** @type {ReportAssessmentRecord[]} */ (results || []);
        const enriched = resultRows.map(ca => {
          const lib = ca.assessment_id ? libraryMap[ca.assessment_id] : undefined;
          return {
            ...ca,
        ...
        completed.sort((a, b) => new Date(b.assessment_date || b.created_date || 0).getTime() - new Date(a.assessment_date || a.created_date || 0).getTime());
  ...
  // The assessments feeding the deterministic outcome comparison table. The
  // clinician's explicit selection wins; otherwise all completed assessments
  // within the chosen date range are used.
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // Build the compatibility HTML locally. It remains a draft until the
      // purpose-specific Core endpoint records a review event.
      const draftHtml = reportHtml || buildReportHtml(reportTemplate, activeSections, sectionContent, client, clinician, dateRange, getOutcomeAssessments());
      const existingCoreMetadata = existingReport?.core_metadata;
      const hasImmutableCoreLineage = Boolean(
        existingCoreMetadata?.artifactId
        || existingCoreMetadata?.contentFingerprint
        || existingCoreMetadata?.releaseAuthorizationEventId
        || existingCoreMetadata?.releaseBinding
      );
      const draftRevisionId = globalThis.crypto?.randomUUID?.() || `legacy-draft-${Date.now()}`;
      ...
      const payload = {
        ...
        // Additive: which sections were AI-drafted, alongside the durable
        // per-section tag already embedded in report_html. Rides in the
        // schemaless blob, so an older image neither drops it nor chokes.
        ai_assisted_sections: activeSections.filter((s) => sectionContent[`${s}_ai_drafted`]),
        report_html: draftHtml,
        status: "draft",
        // Deliberately construct new draft metadata. Never spread an earlier
        // Core artifact, source manifest, claim ledger, fingerprint, approval
        // or release receipt onto edited content.
        core_metadata: {
      ...
      if (
        ...
        && !hasImmutableCoreLineage
      ) {
        await base44.entities.SavedReport.update(existingReport.id, payload);
        toast.success("Draft updated. Approval and export remain locked pending Core review.");
      } else {
        await base44.entities.SavedReport.create(payload);
        toast.success("Draft saved. Approval and export remain locked pending Core review.");
      }
      onClose(true);
    } catch (error) {
  ...
  const DraftReviewStep = () => (
    <div className="space-y-3">
      <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900" role="status">
        <strong>Draft preview only.</strong> Approval, release and print/PDF export remain locked until a purpose-specific Core review event is connected and recorded.
      </div>
      <ReviewExport reportHtml={reportHtml} client={client} clinician={clinician} onEditHtml={setReportHtml} />
    </div>
  );
  ...
  const getStepContent = () => {
    if (isEditing) {
      ...
      if (step === 1) return <DraftReviewStep />;
    }
    switch (step) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => onClose(false)}>
      <ReportDialogContent className="max-w-4xl w-full max-h-[92vh] flex flex-col p-0 gap-0">
        <DialogHeader className="px-6 pt-5 pb-3 border-b border-slate-200 flex-shrink-0">
          <ReportDialogTitle className="text-lg font-bold text-slate-800">
          ...
          <div className="mt-2 space-y-1">
            ...
            <ReportProgress value={progress} className="h-1.5" />
          </div>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-4">
          {loadingContext && step >= (isEditing ? 0 : 2) && (
            <div className="flex items-center gap-2 text-xs text-blue-600 bg-blue-50 rounded px-3 py-2 mt-3">
              ...
              Loading local source records for clinician reference...
            </div>
          )}
          {getStepContent()}
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 flex-shrink-0 bg-white">
          ...
          <div className="flex gap-2">
            {isLastStep ? (
              <>
                <Button variant="outline" disabled title="Export requires a recorded Core review event.">
                  <Printer className="w-4 h-4 mr-1" /> Export locked
                </Button>
