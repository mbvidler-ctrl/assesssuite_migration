...
export default function AstrandTestRunner({ onSave, onClose, initialData, client }) {
  ...
  const calc = useMemo(() => {
    ...
    if (!hr5 || hr5 < 60 || hr5 > 170) errors.push("Minute 5 HR must be between 60–170 bpm.");
    if (!hr6 || hr6 < 60 || hr6 > 170) errors.push("Minute 6 HR must be between 60–170 bpm.");
    ...
    // Caution flags
    if (steadyStateHR < 120) warnings.push("Average steady-state HR < 120 bpm — result may be less reliable.");
    if (steadyStateHR > 170) warnings.push("Average steady-state HR > 170 bpm — result may be less reliable.");
    ...
    if (hr4 && Math.abs(hr4 - hr5) > 5) warnings.push("Minute 4 and 5 HR differ by >5 bpm — steady state may have been entered late.");
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-3xl bg-white max-h-[90vh] overflow-y-auto">
        <CardHeader className="flex flex-row items-start justify-between space-y-0">
          <div>
            <CardTitle className="text-xl font-bold">Åstrand-Rhyming Cycle Ergometer Test</CardTitle>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800 space-y-1">
            ...
            <p><strong>Setup:</strong> Calibrated cycle ergometer. Cadence: <strong>50 rpm</strong>. Workload: 50–100 W (women), 100–150 W (men) — adjust to target HR 125–170 bpm by minute 2–3.</p>
            <p><strong>Protocol:</strong> 2 min warm-up → steady workload. Record HR at end of minutes 4, 5, and 6. Steady state = |HR5 − HR6| ≤ 5 bpm.</p>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 text-sm">
            <p className="font-semibold text-slate-700 mb-2">📊 VO2max Norms (ml/kg/min) — ACSM</p>
            <table className="w-full text-xs border border-slate-300 rounded">
              <thead className="bg-slate-200">
                <tr>
                  <th className="p-2 text-left">Category</th>
                  <th className="p-2 text-center">Men 20–39</th>
                  <th className="p-2 text-center">Men 40–59</th>
                  <th className="p-2 text-center">Women 20–39</th>
                  <th className="p-2 text-center">Women 40–59</th>
                </tr>
              </thead>
              <tbody>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            {data.hr_minute5 && data.hr_minute6 && (
              <p className="text-sm text-green-700 mt-2">
                ...
                {' · '}Diff: <strong>{Math.abs(parseFloat(data.hr_minute6) - parseFloat(data.hr_minute5))} bpm</strong>
                {Math.abs(parseFloat(data.hr_minute6) - parseFloat(data.hr_minute5)) <= 5
                  ? <span className="text-green-600"> ✓ Steady state achieved</span>
                  : <span className="text-red-600"> ✗ Not in steady state</span>}
              </p>
            )}
          </div>
          ...
          {calc.valid && (
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-300 rounded-lg p-5 space-y-2">
              ...
              <p className="text-sm text-slate-600">
                ...
                {' · '}Steady-state HR: <strong>{calc.steadyStateHR} bpm</strong>
                {' · '}Age-correction: <strong>×{calc.ageFactor}</strong>
              </p>
              <div className="text-xs text-slate-500 pt-1 border-t border-blue-200">
                VO2submax = {calc.vo2Submax} ml/kg/min → Uncorrected VO2max = {calc.vo2Uncorrected} ml/kg/min → Corrected = {calc.vo2Final} ml/kg/min
              </div>
            </div>
          )}
          ...
          <div className="bg-slate-100 border border-slate-200 rounded-lg p-3 text-xs text-slate-600">
            ...
            <p>Åstrand PO & Rhyming I. (1954). A nomogram for calculation of aerobic capacity from pulse rate during submaximal work. <em>J Appl Physiol, 7</em>(2), 218–221.</p>
