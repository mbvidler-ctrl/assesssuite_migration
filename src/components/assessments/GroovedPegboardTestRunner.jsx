...
const PHASE_LABELS = {
  dominant: "Peg Only — Dominant Hand",
  non_dominant: "Peg Only — Non-Dominant Hand",
  assembly: "Assembly (Peg + Washer + Nut) — Both Hands",
...
const PHASE_INSTRUCTIONS = {
  ...
  assembly: "Using both hands, insert a peg into a hole, then place a washer over the peg, then a collar. Complete as many assemblies as possible in 30 seconds. Count total pieces placed (max = pegs × 4).",
};
...
export default function GroovedPegboardTestRunner({ client, onSave, onClose }) {
      ...
      intervalRef.current = setInterval(() => {
        setElapsed(prev => {
          ...
          if (prev + 1 >= limit) {
            ...
            if (isAssembly) toast.info("30 seconds complete — record pieces assembled.");
            else toast.warning("5-minute limit reached — trial discontinued.");
            return limit;
          }
          return prev + 1;
        });
  ...
  const handleStopInternal = (time) => {
    if (phase === "dominant" || phase === "non_dominant") {
      ...
      setPhase("done");
      toast.success("Assembly trial complete — all subtests done!");
    }
    setElapsed(0);
  };
      ...
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-6 border-b bg-gradient-to-r from-violet-50 to-blue-50 rounded-t-xl flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-600 text-sm mt-1">Hand dexterity & fine motor coordination — 3 subtests</p>
          </div>
        ...
        <div className="p-6 space-y-5">
            ...
            {showInfo && (
              <div className="p-4 text-sm text-slate-700 space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <ul className="list-disc list-inside space-y-0.5 text-slate-600">
                    <li><strong>Peg Only (Dominant):</strong> Place all 25 pegs using dominant hand — time to completion (max 5 min)</li>
                    <li><strong>Peg Only (Non-Dominant):</strong> Same task with non-dominant hand</li>
                    <li><strong>Assembly:</strong> Both hands — insert peg, add washer, add nut, repeat for 30 seconds. Count total pieces placed.</li>
                  </ul>
                </div>
          ...
          {phase !== "done" && (
            <div className="bg-slate-50 border rounded-lg p-5 text-center space-y-2">
              ...
              {!isAssembly && elapsed >= 270 && <p className="text-red-600 text-sm font-medium">⚠ Approaching 5-minute limit</p>}
          ...
          <div className="space-y-3">
            ...
            {["dominant", "non_dominant"].map((p) => {
              const label = p === "dominant"
                ? `Peg Only — Dominant (${dominantHand === "right" ? "Right" : "Left"})`
                : `Peg Only — Non-Dominant (${dominantHand === "right" ? "Left" : "Right"})`;
              const res = results[p];
            ...
            <div className={`border rounded-lg p-4 ${results.assembly.time !== null ? "border-green-300 bg-green-50" : PHASES.indexOf(phase) < PHASES.indexOf("assembly") ? "opacity-40" : "border-amber-200 bg-amber-50"}`}>
              <div className="flex justify-between items-center mb-2">
                <p className="font-semibold text-sm">Assembly — Peg + Washer + Nut (30s, Both Hands)</p>
          ...
          {phase === "done" && (
            <div className="bg-green-50 border border-green-300 rounded-lg p-3 text-center">
              <p className="text-green-800 font-semibold">✓ All trials complete — ready to save</p>
            </div>
          )}
