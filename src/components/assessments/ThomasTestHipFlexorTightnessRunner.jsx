...
function hipSeverity(angle) {
  ...
  if (a <= 10) return "Mild (1â€“10Â°)";
  if (a <= 20) return "Moderate (11â€“20Â°)";
  return "Severe (>20Â°)";
}
...
function kneeSeverity(angle) {
  ...
  if (a >= 70) return "Mild (70â€“79Â°)";
  if (a >= 50) return "Moderate (50â€“69Â°)";
  return "Severe (<50Â°)";
}
...
function buildInterpretation(data) {
  ...
  if (!isNaN(hip) && hip > 0) {
    ...
    findings.push(`Hip flexion of ${hip}Â° above table â€” ${hipSeverity(hip)} iliopsoas/hip flexor tightness.`);
  } else if (!isNaN(hip)) {
    negativeStructures.push("Iliopsoas");
    findings.push("Thigh rests flat â€” iliopsoas within normal limits.");
  }

  // Rectus femoris
  if (!isNaN(knee) && knee < 80) {
    positiveStructures.push("Rectus femoris");
    findings.push(`Knee flexion ${knee}Â° (normal â‰¥80Â°) â€” ${kneeSeverity(knee)} rectus femoris tightness.`);
  } else if (!isNaN(knee)) {
    negativeStructures.push("Rectus femoris");
    findings.push(`Knee flexion ${knee}Â° â€” rectus femoris within normal limits.`);
  }

  // TFL / ITB
  if (data.abduction && data.abduction !== "none") {
    positiveStructures.push("TFL / ITB");
    findings.push(`${data.abduction.charAt(0).toUpperCase() + data.abduction.slice(1)} hip abduction observed â€” indicates TFL/ITB tightness.`);
  } else if (data.abduction === "none") {
    negativeStructures.push("TFL / ITB");
    findings.push("No hip abduction â€” TFL/ITB within normal limits.");
  }
  ...
  if (data.externalRotation && data.externalRotation !== "none") {
    ...
    findings.push(`${data.externalRotation.charAt(0).toUpperCase() + data.externalRotation.slice(1)} external rotation present â€” may indicate sartorius tightness or compensatory external rotator restriction.`);
  }

  // Compensation
  if (data.pelvicCompensation === "present") {
    findings.push("Pelvic compensation observed â€” test validity may be reduced; lumbar spine position must be controlled.");
  }
  if (data.lumbarExtension === "present") {
    findings.push("Lumbar extension compensation â€” anterior pelvic tilt may be masking hip flexor tightness. Reassess with stricter pelvic control.");
  }

  // Pain
  if (data.painPresent === "yes") {
    findings.push(`Pain provocation: ${data.painLocation || "location unspecified"} â€” severity ${data.painSeverity}/10. Interpret with caution; pain may limit test validity.`);
  }
...
function buildRecommendations(data) {
  ...
  if (!isNaN(hip) && hip > 0) {
    recs.push("Iliopsoas stretching: kneeling hip flexor stretch, supine hip flexor elongation, progressive holds 30â€“60s Ã— 3â€“5 sets.");
  ...
  recs.push("Reassess in 4â€“6 weeks following targeted stretching and strength program.");
...
function buildRedFlags(data) {
  ...
  if (data.painPresent === "yes" && !isNaN(pain) && pain >= 7) flags.push("Strong pain provocation (â‰¥7/10) â€” test validity compromised. Investigate pathology before stretching.");
  if (data.pelvicCompensation === "present" && data.lumbarExtension === "present") flags.push("Unable to maintain pelvic/lumbar neutral â€” test result may be invalid. Repeat with strict positioning or use alternative assessment.");
  const hip = parseFloat(data.hipAngle);
  const hipR = parseFloat(data.hipAngleR);
  if (!isNaN(hip) && !isNaN(hipR) && Math.abs(hip - hipR) >= 10) flags.push(`Significant bilateral asymmetry in hip flexion (${Math.abs(hip - hipR).toFixed(0)}Â°) â€” investigate unilateral pathology.`);
  if (data.setupConfirmed === false) flags.push("Test setup checklist not confirmed â€” results may be unreliable.");
  return flags;
}
...
// â”€â”€â”€ Side data init â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Side Form â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div className="border border-slate-200 rounded-xl overflow-hidden">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-xs font-semibold text-slate-600">Hip Flexion Angle (Â°)</Label>
            <p className="text-xs text-slate-400">Normal = 0Â° (flat on table)</p>
            ...
            {!isNaN(hip) && hip > 0 && (
              <p className="text-xs font-medium text-orange-600 mt-1">âš  {hipSeverity(data.hipAngle)} tightness</p>
            )}
            {!isNaN(hip) && hip <= 0 && <p className="text-xs text-green-600 mt-1">âœ“ Normal</p>}
          </div>
          <div>
            <Label className="text-xs font-semibold text-slate-600">Knee Flexion Angle (Â°)</Label>
            <p className="text-xs text-slate-400">Normal â‰¥ 80Â°</p>
            ...
            {!isNaN(knee) && knee < 80 && (
              <p className="text-xs font-medium text-orange-600 mt-1">âš  {kneeSeverity(data.kneeAngle)} RF tightness</p>
            )}
            {!isNaN(knee) && knee >= 80 && <p className="text-xs text-green-600 mt-1">âœ“ Normal</p>}
          </div>
        </div>
        ...
        <div className="border border-slate-100 rounded-lg p-3 space-y-3 bg-slate-50">
          ...
          {data.painPresent === "yes" && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label className="text-xs text-slate-500">Pain Severity (0â€“10)</Label>
                <Input type="number" min={0} max={10} value={data.painSeverity} onChange={e => update("painSeverity", e.target.value)}
                  placeholder="0â€“10" className="mt-1 h-8 text-xs" />
              </div>
            </div>
          )}
        </div>
        ...
        {redFlags.length > 0 && (
          <div className="bg-red-50 border border-red-300 rounded-lg p-3 space-y-1">
            ...
            {redFlags.map((f, i) => <p key={i} className="text-xs text-red-700">â€¢ {f}</p>)}
          </div>
        )}
        ...
        {(interp.positiveStructures.length > 0 || interp.negativeStructures.length > 0) && (
          <div className={`rounded-lg border px-3 py-3 text-xs space-y-1 ${hasFindings ? "bg-orange-50 border-orange-200" : "bg-green-50 border-green-200"}`}>
            ...
            {interp.findings.map((f, i) => <p key={i} className="text-slate-700 mt-1">â€¢ {f}</p>)}
          </div>
        )}
      </div>
    </div>
...
function SummaryCard({ sides, testMode }) {
  ...
  const iliopsoas = (d) => parseFloat(d.hipAngle) > 0 ? "Tight" : parseFloat(d.hipAngle) <= 0 ? "Normal" : "â€”";
  const rectusFem = (d) => parseFloat(d.kneeAngle) < 80 ? "Tight" : parseFloat(d.kneeAngle) >= 80 ? "Normal" : "â€”";
  const tfl = (d) => d.abduction && d.abduction !== "none" ? "Tight" : d.abduction === "none" ? "Normal" : "â€”";
  const compensation = (d) => d.pelvicCompensation === "present" || d.lumbarExtension === "present" ? "Yes" : "No";
  const painProv = (d) => d.painPresent === "yes" ? "Yes" : d.painPresent === "no" ? "No" : "â€”";
  ...
  const asymLabel = asymmetry === null ? "â€”" : asymmetry < 5 ? "Minimal" : asymmetry < 10 ? "Mild" : asymmetry < 15 ? "Moderate" : "Significant";
          ...
          <tbody>
            ...
            {testMode === "bilateral" && asymmetry !== null && (
              <tr className="border-b border-slate-100">
                ...
                <td colSpan={2} className={`py-1.5 text-center text-xs ${asymmetry >= 10 ? "text-red-700 font-semibold" : "text-slate-600"}`}>{asymLabel} ({asymmetry.toFixed(0)}Â°)</td>
              </tr>
            )}
          </tbody>
        </table>
...
export default function ThomasTestHipFlexorTightnessRunner({ client, onSave, onClose }) {
  ...
  const buildSoapForSide = (label, d) => {
    ...
    return [
      `  ${label} Side:`,
      d.hipAngle ? `    Hip Flexion: ${d.hipAngle}Â° â€” ${parseFloat(d.hipAngle) > 0 ? hipSeverity(d.hipAngle) + " tightness" : "Normal"}` : "",
      d.kneeAngle ? `    Knee Flexion: ${d.kneeAngle}Â° â€” ${parseFloat(d.kneeAngle) < 80 ? kneeSeverity(d.kneeAngle) + " RF restriction" : "Normal"}` : "",
      ...
      d.painPresent === "yes" ? `    Pain: Yes â€” ${d.painLocation || "?"} ${d.painSeverity}/10` : "    Pain: No",
      interp.positiveStructures.length > 0 ? `    Positive: ${interp.positiveStructures.join(", ")}` : "",
      interp.findings.map(f => `    â†’ ${f}`).join("\n"),
    ].filter(Boolean).join("\n");
  };
  ...
  const handleSave = () => {
    ...
    const soapLines = [
      `â€¢ Thomas Test (Hip Flexor Tightness Assessment)`,
      ...
      redFlags.length > 0 ? `  âš  Red Flags: ${redFlags.join("; ")}` : "",
    ...
    <div className="space-y-5 p-1">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Hip flexor, rectus femoris &amp; TFL/ITB tightness â€” bilateral clinical assessment</p>
        </div>
      ...
      <div className="border border-blue-200 rounded-lg overflow-hidden">
        <button onClick={() => setShowInstructions(v => !v)}
          ...
          ðŸ“‹ Protocol &amp; Clinician Instructions
        ...
        {showInstructions && (
          <div className="bg-white p-4 text-sm space-y-3">
            <div className="grid md:grid-cols-2 gap-3">
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-xs space-y-1">
                ...
                <ul className="list-disc pl-4 space-y-1 text-slate-600">
                  ...
                  <li>Hold untested knee â€” flattens lumbar spine</li>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs space-y-1">
                ...
                <ul className="list-disc pl-4 space-y-1 text-blue-700">
                  <li><strong>Hip flexion &gt;0Â°:</strong> Iliopsoas tight</li>
                  <li><strong>Knee flexion &lt;80Â°:</strong> Rectus femoris tight</li>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs">
              ...
              <p className="text-amber-700 mt-1"><strong>Hip Flexion:</strong> Mild 1â€“10Â° | Moderate 11â€“20Â° | Severe &gt;20Â°</p>
              <p className="text-amber-700"><strong>Knee Flexion:</strong> Mild 70â€“79Â° | Moderate 50â€“69Â° | Severe &lt;50Â°</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="border border-slate-200 rounded-lg p-4 space-y-3">
        <p className="text-sm font-bold text-slate-800">âœ… Test Setup Checklist <span className="text-xs text-slate-400 font-normal">(required before saving)</span></p>
      ...
      {(() => {
        ...
        return recs.length > 0 ? (
          <div className="border border-indigo-200 rounded-lg p-4 bg-indigo-50">
            <p className="text-sm font-bold text-indigo-800 mb-2">ðŸ’¡ Clinical Recommendations</p>
            <ul className="space-y-1">
              {recs.map((r, i) => <li key={i} className="text-xs text-indigo-700">â€¢ {r}</li>)}
            </ul>
          </div>
        ) : null;
      })()}
      ...
      <div className="bg-slate-100 border border-slate-200 rounded-lg p-3 text-xs text-slate-600 space-y-1">
        ...
        <p>1. Harvey D. (1998). Assessment of the flexibility of elite athletes using the modified Thomas test. <em>BJSM</em>, 32(1), 68â€“70.</p>
        <p>2. Peeler J &amp; Anderson JE. (2008). Reliability of the Thomas Test for assessing range of motion about the hip. <em>Physical Therapy in Sport</em>, 9(1), 14â€“21.</p>
        <p>3. Clapis PA, Davis SM, Davis RO. (2008). Reliability of inclinometer and goniometric measurements of hip extension flexibility using the modified Thomas test. <em>Physiotherapy Theory and Practice</em>, 24(2), 135â€“141.</p>
        <p>4. Ober FR. (1936). The role of the iliotibial band and fascia lata as a factor in causation of low-back disabilities. <em>JBJS</em>, 18(1), 105â€“110.</p>
      </div>
