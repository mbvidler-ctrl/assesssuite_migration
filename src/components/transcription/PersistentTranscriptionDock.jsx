      ...
      <CardContent className="max-h-[70vh] space-y-4 overflow-y-auto">
        ...
        {active && (
          <>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {['paused', 'recoverable'].includes(transcription.phase) && <Button type="button" variant="outline" onClick={transcription.resume}><Play className="mr-2 h-4 w-4" />Resume recording</Button>}
              {transcription.phase === 'recoverable' && <Button type="button" variant="outline" onClick={transcription.retry}><RotateCcw className="mr-2 h-4 w-4" />Retry saved parts</Button>}
              {['recording', 'paused', 'recoverable'].includes(transcription.phase) && <Button type="button" className="bg-teal-800 hover:bg-teal-900" onClick={transcription.finish}><Square className="mr-2 h-4 w-4" />Finish &amp; prepare note</Button>}
        ...
        {transcription.session?.artifacts && (
          <div className="space-y-3 rounded-lg border border-emerald-200 bg-emerald-50/50 p-3">
            ...
            <div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => copyText(soapText, 'SOAP draft copied for clinician review.')}><Clipboard className="mr-1 h-3.5 w-3.5" />Copy SOAP draft</Button><Button type="button" size="sm" variant="ghost" onClick={transcription.reset}>Close completed session</Button></div>
          </div>
        )}
      </CardContent>
    </Card>
